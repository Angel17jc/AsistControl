...
| `Notification`                          | Pospuesta: hoy las alertas son en tiempo real por WebSocket; se añadirá con el módulo de notificaciones persistentes (roadmap).                                                               |
...
Las marcaciones y jornadas son registros laborales: no se eliminan físicamente. Los datos biométricos **no** se almacenan (solo el ID de enrolamiento; las huellas/rostros permanecen en el equipo).
